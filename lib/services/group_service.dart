...
class GroupService extends ChangeNotifier {
  ...
  Future<void> loadUserGroups() async {
    ...
    try {
      ...
      final userGroups = List<String>.from(userData['groups'] ?? []);
      ...
      for (final participantId in split.participants) {
        if (participantId == split.paidBy) {
          ...
          updatedBalances[participantId] = 
              (updatedBalances[participantId] ?? 0) + split.totalAmount - split.splitAmounts[participantId]!;
        } else {
          // Others get debited
          updatedBalances[participantId] = 
              (updatedBalances[participantId] ?? 0) - split.splitAmounts[participantId]!;
        }
      }
  ...
  Future<void> loadUserSplits() async {
    ...
    try {
      ...
      _splits = query.docs
          .map((doc) => SplitModel.fromMap(doc.data()))
          .toList();
  ...
  Future<void> _updateGroupBalancesForSettlement(String groupId, SettlementModel settlement) async {
    try {
      final groupDoc = await _firestore.collection('groups').doc(groupId).get();
      if (!groupDoc.exists) return;
  ...
  List<SplitModel> getUserOwingSplits(String userId) {
    return _splits.where((s) => 
        s.paidBy == userId && 
        s.participants.any((p) => p != userId && s.getRemainingAmount(p) > 0)
    ).toList();
  }

  double getTotalAmountOwed(String userId) {
    return getUserOwedSplits(userId)
        .fold(0.0, (sum, split) => sum + split.getRemainingAmount(userId));
  }

  double getTotalAmountOwing(String userId) {
    return getUserOwingSplits(userId)
        .fold(0.0, (sum, split) => sum + split.participants
            .where((p) => p != userId)
